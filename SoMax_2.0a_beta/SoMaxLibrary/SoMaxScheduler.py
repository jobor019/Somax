...
class SomaxScheduler(object):
    ...
    def write(self, player, time, *args):  # writes events in scheduler
        ...
        self.logger.debug("[write] Wrote event for player {} at time {} with content {}.".format(player, time, args))
        self.logger.debug("[write] Number of events in the timeline are {}".format(len(self.timeline)))
