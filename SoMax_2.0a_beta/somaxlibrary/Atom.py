...
class Atom(object):
    def __init__(self, name="atom", weight=1.0, label_type=AbstractLabel, contents_type=AbstractContents,
        ...
        self.memorySpace = memory_type(label_type=label_type, contents_type=contents_type, event_type=event_type)
        self.name = name
        self.active = False
        if memory_file != None:
            self.read(memory_file, label_type=self.label_type, contents_type=self.contents_type,
    ...
    def read(self, filez, memory_type=None, label_type=AbstractLabel,
        ...
        # read file
        self.logger.info("Atom {} reading file {}...".format(self.name, filez))
        success = self.memorySpace.read(filez)
        if success == False:
            # TODO: Exception or log.error?
            raise Exception("[ERROR] failed to load the file ", filez)
        else:
            self.logger.info("Atom {} file {} loaded".format(self.name, filez))
        # set current file
    ...
    # own copy method
    def copy(self, name):
        atom = Atom(name=name, weight=self.weight, label_type=self.memorySpace.label_type,
                    contents_type=self.memorySpace.contents_type, event_type=self.memorySpace.event_type,
                    activity_type=self.activity_type, memory_type=self.memory_type)
        atom.memorySpace = self.memory_type(self.memorySpace.get_dates_list(), self.memorySpace.get_events_list(),
                                            label_type=self.memorySpace.label_type,
                                            contents_type=self.memorySpace.contents_type,
                                            event_type=self.memorySpace.event_type)
