    ...
    def call(self, string: str):
       ...
       bracket, parenthesis or brace. Similarly, strings containing multiple words can created using single quotes.
        ...
        try:
            ...
            self.send_warning(f"Error: {e}. The key of a dict must be a primitive or a tuple.")
            return
        ...
        try:
            ...
            self.send_warning(f"Error: {e}. Likely, the first argument does match a function name.")
            return

        try:
            func(*args, **kwargs)
        except TypeError as e:
            self.send_warning(f"Error: {e}. The arguments for function {func.__name__} are: {func.__code__.co_varnames[1:]}")
            raise
